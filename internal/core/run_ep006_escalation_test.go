...
func TestRun_toolMayEscalate_advancesToSecondProvider(t *testing.T) {
	...
	cfg := &config.Config{
		...
		LLMProviders: []config.LLMProvider{
			{Type: "a", Model: "m0", SupportsTools: boolAsPtr(true), DefaultTemperature: 0.3, DefaultMaxTokens: 1024, DefaultResponseFormat: "text"},
			{Type: "b", Model: "m1", SupportsTools: boolAsPtr(true), DefaultTemperature: 0.3, DefaultMaxTokens: 1024, DefaultResponseFormat: "text"},
			{Type: "c", Model: "m2", SupportsTools: boolAsPtr(true), DefaultTemperature: 0.3, DefaultMaxTokens: 1024, DefaultResponseFormat: "text"},
		},
	...
	err := Run(context.Background(), cfg, logger, adapter,
		...
		nil, nil, nil, runner, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
