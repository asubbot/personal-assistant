...
// NodeRunner runs an allowlisted command on a node via SSH (REQ-01.004, REQ-01.005, REQ-01.013).
// When a tool or flow requires node action, core (or tools) calls RunOnNode; implementation checks cmdsafe, allowlist, then runs via SSH.
