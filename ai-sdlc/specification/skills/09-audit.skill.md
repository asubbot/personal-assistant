...
- **Test and coverage outputs:** Run tests and checks as defined in the project (e.g. `go test`, lint, static analysis); use terminal output or project/CI artefacts. If the project documents commands (e.g. in README, Makefile, strategy.md), use those.
...
2. **Run tests and checks** — Execute tests and quality checks as defined in the project (e.g. build, unit tests, lint, static analysis such as `go vet`, staticcheck, or project-specific tools). Capture pass/fail and coverage if available.
...
- **Test results and coverage** — Commands run, pass/fail, and coverage metrics if available. Reference which AC (AC-XXX) are covered by which tests where applicable.
...
- **Quality gate** — Lint and static analysis: tools/commands used (e.g. `go vet`, staticcheck, golangci-lint — as defined in the project or strategy.md), and result (pass/fail or list of issues).
