...
**Output:** Refined requirements and glossary in ai-sdlc-artefacts/epics/<epic-id>/01-02-requirements.md and ai-sdlc-artefacts/epics/<epic-id>/08-user-stories.md
...
**EARS patterns** (every requirement MUST follow exactly one): Ubiquitous, Event-driven, State-driven, Unwanted event, Optional feature, Complex (clause order: WHERE → WHILE → WHEN/IF → THE → SHALL).

**INCOSE rules** (every requirement MUST comply): Active voice, no vague terms, no escape clauses, no negative "SHALL not", one thought per requirement, explicit and measurable conditions, consistent terminology, no pronouns, no absolutes, solution-free, realistic tolerances.
...
- Draft refinements first; show the user (e.g. section by section). Update ai-sdlc-artefacts/epics/<epic-id>/01-02-requirements.md only when the user explicitly approves (e.g. "lgtm", "save").
