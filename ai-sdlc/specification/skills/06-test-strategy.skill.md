...
**Output:** ai-sdlc-artefacts/epics/<epic-id>/06-test-strategy.md, ai-sdlc-artefacts/epics/<epic-id>/06-manual-test-plan.md
...
**Goal:** Produce the test strategy document: test levels and definitions (unit, integration, component, E2E, manual), mapping of AC to recommended levels, pyramid summary, special topics (e.g. secret leakage), and link to ai-sdlc-artefacts/epics/<epic-id>/15-current-coverage.md.
...
- Draft the strategy first; show the user (e.g. section by section). Update ai-sdlc-artefacts/epics/<epic-id>/06-test-strategy.md and ai-sdlc-artefacts/epics/<epic-id>/06-manual-test-plan.md only when the user explicitly approves (e.g. "lgtm", "save").
...
**Rules:** Use English. Keep traceability to AC and REQ. Do not leave risk areas (e.g. secrets) without a test approach.
