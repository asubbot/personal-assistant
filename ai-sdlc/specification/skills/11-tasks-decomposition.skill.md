...
**Output:** ai-sdlc-artefacts/epics/<epic-id>/11-12-implementation-plan.md (task breakdown)
...
- Each task: clear objective (writing, modifying, or testing code); sub-bullets with details; **traceability block** with three link lines:
  - **_Requirements:_** REQ-X … (links to ai-sdlc-artefacts/epics/<epic-id>/01-02-requirements.md)
  - **_User Stories:_** US-X … (links to ai-sdlc-artefacts/epics/<epic-id>/08-user-stories.md)
  - **_Acceptance Criteria:_** AC-X … (links to ai-sdlc-artefacts/epics/<epic-id>/stories/<story-id>/acceptance-criteria.md or epic index 10-acceptance-criteria.md)
...
- Draft the task list first; show the user (e.g. section by section). Update ai-sdlc-artefacts/epics/<epic-id>/11-12-implementation-plan.md only when the user explicitly approves (e.g. "lgtm", "save").
...
- This workflow creates planning artifacts only; actual implementation is done in stage 13.
