...
- **Agentic SDLC:** process definitions live in a **nested clone** at **`ai-sdlc/`** (gitignored; pin in **`ai-sdlc.version`**). Canonical source: [github.com/asubbot/ai-sdlc](https://github.com/asubbot/ai-sdlc). Use **[ai-sdlc/README.md](ai-sdlc/README.md)** as the **directory index**. The **step-by-step pipeline** is in **[ai-sdlc/specification/pipeline.spec.md](ai-sdlc/specification/pipeline.spec.md)** and the **`*.skill.md`** files it maps to. Do **not** commit edits under `ai-sdlc/` — bump the pin and refresh the local checkout instead.
