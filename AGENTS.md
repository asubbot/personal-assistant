...
- **Agentic SDLC:** process definitions live in a **nested clone** at **`ai-sdlc/`** (gitignored; pin in **`ai-sdlc.version`**). Canonical source: [github.com/asubbot/ai-sdlc](https://github.com/asubbot/ai-sdlc). On a fresh clone, check out `ai-sdlc/` per **[docs/installation.md](docs/installation.md)** before **`make build`** / **`make check`** / **`make validate`**. **`make check`** enforces that `ai-sdlc/` HEAD matches the pin; standalone **`make build`** / **`make validate`** do not. Use **[ai-sdlc/README.md](ai-sdlc/README.md)** as the **directory index** (after checkout). The **step-by-step pipeline** is in **[ai-sdlc/specification/pipeline.spec.md](ai-sdlc/specification/pipeline.spec.md)** and the **`*.skill.md`** files it maps to. Do **not** commit edits under `ai-sdlc/` — bump the pin and refresh the local checkout instead.
