...
// Covers AC-01.013, AC-01.014 (US-07), AC-02.013 integration: vector store injects past context into LLM call without read_memory.
...
func TestVectorStore_injectsPastContext(t *testing.T) {
	...
	go func() {
		_ = core.Run(ctx1, cfg, logger, adapter1, []llm.Provider{provider}, []string{"test/default"}, nil, core.SingleStoreMemoryVectors(vecStore), emb, nil, nil, nil, nil, nil)
		close(done1)
	}()
	...
	go func() {
		_ = core.Run(ctx2, cfg, logger, adapter2, []llm.Provider{provider}, []string{"test/default"}, nil, core.SingleStoreMemoryVectors(vecStore), emb, nil, nil, nil, nil, nil)
		close(done2)
	}()
