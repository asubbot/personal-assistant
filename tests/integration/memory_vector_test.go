...
func TestVectorStore_injectsPastContext(t *testing.T) {
	...
	go func() {
		_ = core.Run(ctx1, cfg, logger, adapter1, []llm.Provider{provider}, []string{"test/default"}, nil, vecStore, emb, nil, nil, nil, nil)
		close(done1)
	}()
	...
	go func() {
		_ = core.Run(ctx2, cfg, logger, adapter2, []llm.Provider{provider}, []string{"test/default"}, nil, vecStore, emb, nil, nil, nil, nil)
		close(done2)
	}()
