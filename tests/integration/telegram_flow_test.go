...
func TestTelegramFlow_OneMessage_ReplyWithinTimeout(t *testing.T) {
	...
	go func() {
		_ = core.Run(ctx, cfg, logger, adapter, []llm.Provider{provider}, []string{"test/default"}, nil, nil, nil, nil, nil, nil, nil)
		close(done)
	}()
...
func TestTelegramFlow_DifferentProviderUsedPerRun(t *testing.T) {
	...
	go func() {
		_ = core.Run(ctx1, cfg, logger, adapterA, []llm.Provider{providerA}, []string{"test/default"}, nil, nil, nil, nil, nil, nil, nil)
		close(done1)
	}()
	...
	go func() {
		_ = core.Run(ctx2, cfg, logger, adapterB, []llm.Provider{providerB}, []string{"test/default"}, nil, nil, nil, nil, nil, nil, nil)
		close(done2)
	}()
