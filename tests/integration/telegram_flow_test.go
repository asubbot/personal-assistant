...
func TestTelegramFlow_OneMessage_ReplyWithinTimeout(t *testing.T) {
	...
	go func() {
		_ = core.Run(ctx, cfg, logger, adapter, provider, nil, nil, nil, nil, nil)
		close(done)
	}()
...
func TestTelegramFlow_DifferentProviderUsedPerRun(t *testing.T) {
	...
	go func() {
		_ = core.Run(ctx1, cfg, logger, adapterA, providerA, nil, nil, nil, nil, nil)
		close(done1)
	}()
	...
	go func() {
		_ = core.Run(ctx2, cfg, logger, adapterB, providerB, nil, nil, nil, nil, nil)
		close(done2)
	}()
