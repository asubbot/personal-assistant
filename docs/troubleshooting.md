...
| Allowlist | `command_allowlist_path` relative to `PA_CONFIG_DIR`; patterns may use `*` only once, as the last character (prefix wildcard). The prefix is the whole line minus the final `*` — **a space before `*` is part of the prefix**, so `docker images *` does **not** match bare `docker images` (use `docker images*` if you need both). A lone `*`, `**`, `foo*bar`, etc. cause load failure. Remote commands must match the allowlist **and** the character set (letters, numbers, Mn/Mc, fixed ASCII punctuation including space and `"` — not tab); `;` and similar fail with “not in allowed command character set” before shell-specific wording. |
