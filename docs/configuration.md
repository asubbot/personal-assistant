...
| **`full_lite`** | No | Yes (same rules as full) | No | Merged ranked tools + `always_include`; when **`tools.dynamic_selection.enabled_for_full_lite`** and **`tools.text_based_enabled`**, list is filtered and capped by **`max_tools_for_llm_request`** |
| **`full`** | Yes | Yes | Yes (when runtime skills enabled) | Same merge as EP-017; when **`tools.dynamic_selection.enabled_for_full`**, cap applies after ranking |
