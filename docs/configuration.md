...
- **`conversation_context`** — **required**; `max_dynamic_system_runes` (UTF-8 rune budget for the dynamic system tail: tool instructions, retrieved memory, runtime skills) and `vector_search_top_k` must each be **≥ 1**.
