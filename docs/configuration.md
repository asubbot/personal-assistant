...
- **`intent_classifier`** — **optional** (EP-017); two-stage intent classification to reduce main LLM token consumption for simple messages. When present and **`enabled`** is true: **`heuristic`** (optional) defines `simple_patterns`, `full_patterns` (Go `regexp` strings, case-insensitive; invalid regex fails load), and **`max_simple_len`** (must be ≥ 1; messages longer in runes skip simple tier). **`model_stage`** (optional) configures a cheap classification LLM (`type`, `endpoint`, `api_key_path`, `model`, `default_temperature`, `default_max_tokens` ≥ 1, optional `timeout` as Go duration string e.g. `"5s"`). The cascade runs heuristic first; if ambiguous and model stage is enabled, calls the classification model; on any failure defaults to `full` tier. When disabled or omitted, all messages use the full prompt path. `model_stage.api_key_path` is resolved via `PA_SECRETS_DIR`.
