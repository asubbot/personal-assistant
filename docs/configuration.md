...
- **`write_memory`** — **optional** (EP-016). When present, **`max_append_bytes`** and **`max_file_bytes`** bound each appended entry and the per-day **`notes.md`** file size (validated at load; zeros are normalized to documented defaults). The native **`write_memory`** tool is registered when **`paths.memory_dir`**, the notes vector table (**`vec_notes`**), and the embedding provider are available at startup (same curated profile as semantic memory writes). If the block is omitted, **`write_memory`** is not registered and only **`read_memory`** applies for native memory tools in that profile.
