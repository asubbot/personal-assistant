...
- **Memory:** The assistant’s single store: directory of markdown files (structure by topic/date — define in config; not partitioned by interlocutor per [REQ-018](../../REQUIREMENTS.md#memory-and-indexing)). Read/write by core only; format/schema is part of design ([REQ-006](../../REQUIREMENTS.md#memory-and-indexing)).
