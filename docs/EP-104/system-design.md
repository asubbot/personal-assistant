...
- **Storage:** Long-term memory = the assistant’s single store: directory of markdown files (structure by topic/date from config; not partitioned by interlocutor — [REQ-018](../../REQUIREMENTS.md#memory-and-indexing)). Vector store = pluggable interface; default in-process implementation (see [§3 Components](#3-components-and-interfaces) and [research §4.1 Vector store options](research.md#41-vector-store-options-req-007-pluggable)).
...
| **Memory (MD store)** | Read/write the assistant’s single memory store (markdown files in configured directory layout). | File system; structure by topic/date in config; not partitioned by interlocutor ([REQ-006](../../REQUIREMENTS.md#memory-and-indexing), [REQ-018](../../REQUIREMENTS.md#memory-and-indexing)). |
...
- **Long-term memory:** The assistant’s single store: markdown files under memory_dir; structure (e.g. by topic, date) defined in config; not subdivided by interlocutor ([REQ-006](../../REQUIREMENTS.md#memory-and-indexing), [REQ-018](../../REQUIREMENTS.md#memory-and-indexing)); content and chunking strategy are part of implementation.
