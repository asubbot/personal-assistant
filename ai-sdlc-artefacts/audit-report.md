...
| [EP-028](epics/EP-028/ep-scope.md) | Per-user rate limiting and tier-aware tool round caps                             | CANCELED (Not necessary for one user using model) | —             | —                                                               |
...
- **Quality gate (2026-07-09):** `make validate` **pass** — in-scope **452/452** AC traced (100.0%); **617/617** REQs covered (`validate req all`); **0** EARS errors across 38 in-scope epics (`validate ears all`; 5 NEW/CANCEL skipped). `make check` **pass** on fmt, vet, lint, test-race, test-e2e, coverage, boundaries; ai-sdlc pin **v1.0.4**.
