...
- **Tool call (internal):** After parsing provider response: tool_id, arguments (map). Validated against catalog schema; then template substitution yields command string ([REQ-04.007](ep-requirements.md#validation-and-execution), [REQ-04.009](ep-requirements.md#validation-and-execution)).
