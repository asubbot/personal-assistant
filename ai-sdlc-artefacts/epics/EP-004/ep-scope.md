...
| Term | Definition |
|------|------------|
| **Tool (invocable)** | Stable id, `index_text` (short text for vector index and native tool description), optional `system_prompt` (injected into system when the tool is selected), optional `hermes_prompt` (Hermes tool-list line; defaults to `index_text`), bound to a node, schema for arguments. |
...
| **index_text** | Short text: embedded for tool pre-selection (with id and optional triggers) and used as the native API tool `description`. |
| **system_prompt** | Optional per-tool text appended to the system message when that tool is among the selected tools (native and Hermes). |
| **hermes_prompt** | Optional per-tool text in the Hermes “available tools” block; if empty, `index_text` is used. |
...
| **Tool index (vector)** | Built from id + **index_text** + optional **triggers** (not from system/hermes prompts). Same vector DB as memory, dedicated table (e.g. vec_tools). |
...
- **Tool index at startup:** id + **index_text** + optional **triggers** → embeddings → vec_tools. Up to **1000 tools**; load within **20 seconds** or background build with fallback.
