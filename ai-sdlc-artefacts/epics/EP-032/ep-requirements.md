...
### REQ-32.001 — Register native tool id `search_vector_tool`
...
### REQ-32.002 — Register native tool id `search_vector_skill`
...
### REQ-32.003 — Keep `search_vector_memory` memory-only
...
### REQ-32.004 — Define single block for all vector-search tools
...
### REQ-32.005 — Validate config block with fail-fast semantics
...
### REQ-32.006 — `search_vector_memory` reads limits from unified block
...
### REQ-32.007 — `search_vector_tool` reads limits from unified block
THE PersonalAssistant SHALL initialize `search_vector_tool` limits from `tools.vector_search_tools.search_vector_tool`.

### REQ-32.008 — `search_vector_skill` reads limits from unified block
...
### REQ-32.009 — Reject missing/empty query for new tools
...
### REQ-32.010 — Enforce top_k bounds and deterministic ordering
...
### REQ-32.011 — Return compact bounded snippets with source identifiers
...
### REQ-32.012 — Keep retrieval read-only
...
### REQ-32.013 — Log invocation metadata with redaction
...
### REQ-32.014 — Allow new tool IDs in runtime skill validation
