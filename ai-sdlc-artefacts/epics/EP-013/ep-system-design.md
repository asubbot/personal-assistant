...
| **`systemprompt` helpers** | `TrustPolicy`, `WrapRetrievedContext`, `WrapToolInstructions`, `WrapRuntimeSkills`. Assembly order inside system: trust + personality, then TOOLS (`PA_BEGIN_TOOLS` / `PA_END_TOOLS`), then CONTEXT (`PA_BEGIN_CONTEXT` / `PA_END_CONTEXT`), then SKILLS (`PA_BEGIN_SKILLS` / `PA_END_SKILLS`). | [REQ-13.014](ep-requirements.md#prompt-assembly)–[REQ-13.016](ep-requirements.md#prompt-assembly) |
