...
### REQ-18.001 — Three tiers with documented prompt matrix
...
### REQ-18.002 — `simple` excludes tools, RAG, tail skills, Hermes
...
### REQ-18.003 — `full` matches EP-017 when full-tier dynamic off
...
### REQ-18.004 — `full_lite` skips RAG — **Superseded by EP-036**
...
### REQ-18.005 — `full_lite` keeps session exchanges like `full`
...
### REQ-18.006 — `full_lite` omits runtime skill playbook in tail
...
### REQ-18.007 — `full_lite` includes Hermes when tools present
...
### REQ-18.008 — `full_lite` omits Hermes when no tools
...
### REQ-18.009 — Classifier assigns tier — **Superseded by EP-036**
...
### REQ-18.010 — Model stage prompt lists tiers — **Superseded by EP-036**
...
### REQ-18.011 — Model failure defaults to `full` — **Superseded by EP-036**
...
### REQ-18.012 — Merge `always_include` before cap enforcement
...
### REQ-18.013 — Enforce max tools when dynamic selection applies
...
### REQ-18.014 — Ranked order from tool vector pre-selection when enabled
...
### REQ-18.015 — `full` tier unchanged when full-tier dynamic off
...
### REQ-18.016 — `full_lite` uses fallback list when pre-selection off
...
### REQ-18.017 — `full_lite` uses dynamic path when tools enabled
...
### REQ-18.018 — INFO log tier, tool count, dynamic flag, stage
...
### REQ-18.019 — Reject invalid epic configuration at load
...
### REQ-18.020 — `make check` passes on delivery branch
THE PersonalAssistant SHALL pass `make check` on the branch that delivers this epic.

### REQ-18.021 — Each AC mapped to automated or manual test
