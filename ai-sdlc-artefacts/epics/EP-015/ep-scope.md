...
| **Description**        | After processing one user message in Telegram, append a **plain-text** token summary to the **last** outbound message chunk when the LLM provider reported **non-zero** usage for that turn. All figures come from **API `usage`** fields aggregated across every completion in the turn. |
...
| **Token footer**   | A single trailing line of plain text summarising aggregated usage for the turn, in the prescribed format.                                                                                          |
...
- **Format (exact shape, plain text):** One new line at the end of the last chunk, literal pattern: `Tokens <in+out> (in: <in> / out: <out>)` with non-negative integers and a single space after `Tokens` and after the colon in `in:` and `out:`. Example: `Tokens 1801 (in: 1234 / out: 567)`.
- **Plain text:** The footer line contains **no** Telegram HTML tags and **no** Markdown styling; it is literal digits, spaces, ASCII punctuation as shown. (The chunk may still be sent with the adapter’s existing HTML parse mode for the body; the footer must use characters that remain valid and visually identical in that mode—digits, colons, parentheses, slash.)
...
- For a mocked multi-completion turn with non-zero `usage` on each call, the user sees **one** footer line on the **final** chunk only, with **in** and **out** equal to the **sums** of API fields and **Tokens** equal to their sum.
...
- Rich formatting of the footer (bold, monospace HTML, etc.).
