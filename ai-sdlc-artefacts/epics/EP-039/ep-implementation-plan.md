...
- [x] **1.1** Add `sqlite_store_defaults` to `configRootJSONKeys` (sorted, between `runtime_skills` and `telegram`). Add `SQLiteStoreDefaultsConfig` stub on `Config`.
...
- [x] **1.2** Add `rejectLegacyVectorSearchToolsShape` and `rejectLegacySQLiteReliabilityShape` in `load.go`; call from `prepareConfig` before unmarshal. Add testdata: `vector_search_tools_legacy_rejected.json`, `sqlite_reliability_legacy_rejected.json`.
...
- [x] **2.1** Implement `SQLiteStoreDefaultsConfig`, `SQLiteStoreReliabilityOverride`, `mergeSQLiteStoreReliability`; update `ValidateVectorStoreReliability` / `ValidateJobsStoreReliability` to merge defaults + override.
...
- [x] **2.2** Migrate all `internal/config/testdata/*.json`, `config.examples/`, integration configs: add `sqlite_store_defaults`; shrink store blocks to `foreign_keys` only.
...
- [x] **3.1** Replace `VectorSearchToolsConfig` with defaults + override shape; implement `mergeVectorSearchTool`; update `validateVectorSearchTools` and `VectorSearchToolSettings`.
...
- [x] **3.2** Migrate `tools.vector_search_tools` in all testdata/examples to defaults+overrides shape.
  - _REQ:_ [REQ-39.016](ep-requirements.md#req-39-016--update-repository-configs)
  - _AC:_ [AC-39.011](ep-acceptance-criteria.md#ac-39-011)
...
- [x] **4.1** Add `ToolOutputArtifactsConfig` on `ToolsConfig`; `validateToolOutputArtifacts` + nested key whitelist; reject unknown artifact keys.
...
- [x] **4.2** Core: add `toolResultPromptBytes int` on `conversationHandler`; set from config in `newRunConversationHandler`; convert `truncateToolResultForPrompt` to use handler field; add `ArtifactDirectory(cfg)` helper in config package.
...
- [x] **4.3** Migrate operator block shape in examples (artifact block unchanged fields); add parity + negative tests with `// Covers AC-39.xxx`.
...
- [x] **5.1** Update `docs/configuration.md` with migration tables for all three config areas.
...
- [x] **6.1** Run `make check`; fix any failures; add `ep039_traceability_test.go` if needed for AC coverage.
