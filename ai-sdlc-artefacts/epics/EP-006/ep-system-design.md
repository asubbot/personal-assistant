...
| **escalationpolicy** | Central mapping: classified causes → `toolfailure.NoEscalate` / `MayEscalate` (and future `toolcatalog` kinds / sentinels). | No Telegram, no handler loop; unit-testable ([REQ-06.004](ep-requirements.md#error-classification), [REQ-06.005](ep-requirements.md#error-classification), [REQ-06.015](ep-requirements.md#typed-tool-failures-and-hermes-parse-escalation-inputs), [REQ-06.017](ep-requirements.md#nfr--security-testability-observability)). |
...
- **Imports allowed:** `pa/internal/core/toolfailure`; optionally `pa/internal/toolcatalog` once validation errors expose stable kinds or sentinels (removes substring bridge per [REQ-06.015](ep-requirements.md#typed-tool-failures-and-hermes-parse-escalation-inputs)).
...
| **Escalation policy (`internal/escalationpolicy`)** | Single place for cause → escalation allowance (wrap as `NoEscalate`/`MayEscalate`); table-driven tests; fail-closed for unknown causes ([REQ-06.017](ep-requirements.md#nfr--security-testability-observability)). Hermes / handler **advance** logic stays in core; policy package covers **which errors are qualifying** for catalog and aligned paths. | [REQ-06.004](ep-requirements.md#error-classification), [REQ-06.005](ep-requirements.md#error-classification), [REQ-06.015](ep-requirements.md#typed-tool-failures-and-hermes-parse-escalation-inputs) |
...
- **Typed errors:** `QualifiesForEscalation` uses `errors.As` on `toolfailure.Failure` only ([REQ-06.015](ep-requirements.md#typed-tool-failures-and-hermes-parse-escalation-inputs)). **After Task 8 (implementation plan):** catalog-validation path applies `toolfailure` wrappers via `internal/escalationpolicy` ([REQ-06.017](ep-requirements.md#nfr--security-testability-observability)); until then, an interim `wrapCatalogValidateError` may live in the handler. Node runner may continue to attach typed failures at source or delegate to `escalationpolicy` when consolidated.
...
- **Unit tests:** `internal/escalationpolicy`: table-driven tests for each mapped cause → `NoEscalate`/`MayEscalate` and fail-closed unknown input ([REQ-06.017](ep-requirements.md#nfr--security-testability-observability)). Classification via `toolfailure.Failure` and `QualifiesForEscalation`; untyped errors fail closed; cap enforcement; state transitions for `active_index` and `escalation_count`; baseline resolution from config; Hermes escalation paths where mockable ([REQ-06.013](ep-requirements.md#nfr--security-testability-observability)).
