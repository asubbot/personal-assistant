...
- **Typed errors:** `QualifiesForEscalation` uses `errors.As` on `toolfailure.Failure` only ([REQ-06.015](ep-requirements.md#typed-tool-failures-and-hermes-parse-escalation-inputs)). Catalog-validation path applies `toolfailure` wrappers via `internal/escalationpolicy.WrapCatalogValidateError` ([REQ-06.017](ep-requirements.md#nfr--security-testability-observability)). Node runner attaches typed failures at source (optional future consolidation into `escalationpolicy`).
